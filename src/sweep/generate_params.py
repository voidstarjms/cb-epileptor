...
GRANULARITY = 2
J_values     = np.linspace(0.0, 0.1, GRANULARITY)    # CE (coupling strength)
excite_values     = np.linspace(-4.5, -2.5, GRANULARITY)  # X0 (epileptogenicity)
...
def _job_yaml(base, J, excite, gintra, ginter, seed):
    ...
    out['COUPLING_STRENGTH'] = float(J)
    out['EPOP_BASE_EXCITE']       = float(excite)
    ...
    out['COUPLING_VALS']     = [float(J)]
    out['BASE_EXCITE_VALS']     = [float(excite)]
...
def main():
    ...
    rel_paths = []
    combos = product(excite_values, J_values, gintra_values, ginter_values,
                     range(1, n_realizations + 1))
    for job, (excite, J, gintra, ginter, r) in enumerate(combos, start=1):
        job_yaml = _job_yaml(base, J, excite, gintra, ginter, seed=r)
    ...
    print(f"Grid: J x excite x Gintra x Ginter x realizations = "
          f"{len(J_values)} x {len(excite_values)} x {len(gintra_values)} x "
