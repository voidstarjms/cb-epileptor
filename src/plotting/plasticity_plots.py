...
def plot_plasticity(filepaths: Any, fname: str, params_dict: Dict, title: str, t, x, wpre, u, Ca) -> None:
    ...
    plasticity = (1
        - params_dict['A_LTD'] * np.where(effCa > params_dict['THETA_LTD_START'], 1, 0)
        * np.where(effCa < params_dict['THETA_LTD_END'], 1, 0)
        + params_dict['A_LTP'] * (effCa > params_dict['THETA_LTP_START']))
